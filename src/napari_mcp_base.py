...
class NapariMCPTools:
    ...
    async def add_image(
        ...
        if path:
            ...
            return {"status": "error", "message": "Either path or data must be provided"}
            ...
            with (
                ...
                if parsed.body and isinstance(parsed.body[-1], ast.Expr):
                    ...
                    if len(parsed.body) > 1:
                        ...
                        exec(compile(exec_ast, "<mcp-exec>", "exec"), self._exec_globals, self._exec_globals)
                    last_expr = ast.Expression(body=parsed.body[-1].value)
                    value = eval(compile(last_expr, "<mcp-eval>", "eval"), self._exec_globals, self._exec_globals)
                    result_repr = repr(value)
                else:
                    exec(compile(parsed, "<mcp-exec>", "exec"), self._exec_globals, self._exec_globals)
