...
class MockLayerBuilder:
    ...
    def build(self) -> Mock:
        ...
        layer.colormap = Mock(name=self._colormap) if self._colormap else Mock(name="viridis")
...
class MockViewerBuilder:
    ...
    def with_camera(self, center: List[float], zoom: float, angles: Optional[List[float]] = None) -> 'MockViewerBuilder':
    ...
    def build(self) -> Mock:
        ...
        viewer.window = types.SimpleNamespace(
            qt_viewer=types.SimpleNamespace(
                canvas=types.SimpleNamespace(
                    ...
                    size=Mock(return_value=types.SimpleNamespace(
        ...
        def add_image(data, **kwargs):
            layer = MockLayerBuilder().with_name(kwargs.get("name", "image")).with_data(data).build()
            viewer.layers.append(layer)
            return layer
        
        def add_points(data, **kwargs):
            layer = MockLayerBuilder().with_name(kwargs.get("name", "points")).with_data(data).with_type("Points").build()
            viewer.layers.append(layer)
            return layer
        
        def add_labels(data, **kwargs):
            layer = MockLayerBuilder().with_name(kwargs.get("name", "labels")).with_data(data).with_type("Labels").build()
            viewer.layers.append(layer)
            return layer
...
class MockLayersCollection:
    ...
    def __init__(self, initial_layers: Optional[List[Any]] = None):
...
def assert_mock_viewer_state(viewer: Mock, expected_layers: int = 0, expected_title: str = ""):
