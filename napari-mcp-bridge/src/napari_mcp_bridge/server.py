...
class NapariBridgeServer:
    ...
    def _run_in_main(self, func):
        ...
        def wrapper(*args, **kwargs):
            return self.qt_bridge.run_in_main_thread(
    ...
    def _setup_tools(self):
        ...
        async def session_information() -> dict[str, Any]:
            ...
            def get_info():
                viewer_info = {
                    ...
                    "camera_angles": list(self.viewer.camera.angles) if self.viewer.camera.angles else [],
        ...
        async def add_image(
            ...
            if path:
                ...
                return {"status": "error", "message": "Either data or path must be provided"}
                    ...
                    with (
                        ...
                        if parsed.body and isinstance(parsed.body[-1], ast.Expr):
                            if len(parsed.body) > 1:
                                ...
                                exec(compile(exec_ast, "<bridge-exec>", "exec"), self._exec_globals)
                            last_expr = ast.Expression(body=parsed.body[-1].value)
                            value = eval(compile(last_expr, "<bridge-eval>", "eval"), self._exec_globals)
                            result_repr = repr(value)
                        else:
                            exec(compile(parsed, "<bridge-exec>", "exec"), self._exec_globals)
